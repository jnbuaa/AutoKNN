# -*- coding: utf-8 -*-
"""
Created on Thu Apr 25 17:03:37 2019
...
class DDPG():
    ...
    def _build_actor(self):
        """Actor model.
        """
        traffic_input = Input(shape=(6, 257), name='traffic_input')
        ...
        x = Dense(self.env.seknn.link, activation='tanh')(x)
    ...
    def _build_critic(self):
        """Critic model.
        """
        traffic_input = Input(shape=(6, 257), name='state_input')
        error_input = Input(shape=(257,), name='traffic_input')
        action_input = Input(shape=(257,), name='action_input')
    ...
    def actor_optimizer(self):
        ...
        self.action_gradient = tf.placeholder(tf.float32, shape=(None, self.env.seknn.link))
    ...
    def get_action(self, X):
        ...
        if self.epsilon >= np.random.rand():
            action = -self.env.diff
        else:
            action = self.actor.predict([np.asarray(X[0]).reshape(-1, 6, 257), np.asarray(X[1]).reshape(-1, 257)])
            
        action = action.reshape(257,)

        # add randomness to action selection for exploration
        noise = max(self.epsilon, 0) * self.OU(action).reshape(257,)
    ...
    def process_batch(self, batch):
        ...
        next_traffic = np.array([n[0] for n in next_states]).reshape(-1, 6, 257)
        ...
        q = self.target_critic.predict([next_traffic, next_error, np.asarray(next_actions).reshape(-1, 257)])
...
#        loss = self.critic.train_on_batch([X1, X2], y)
    ...
    def train(self, episode, batch):
        ...
        for i in range(episode):
            ...
            for j in range(200):
                ...
                observation, reward, done, _ = self.env.step(action, self.env.pointer)
                # add data to experience replay.
                reward_sum += reward
                self.remember(x, action, reward, observation, done)

                if len(self.memory_buffer) > 1000: # batch:
    ...
    def play(self,episode):
        ...
        while self.env.pointer < len(self.env.testY_nofilt):
            ...
            x = observation
            x_traffic = np.array(x[0]).reshape(-1, 6, 257)
            x_error = np.array(x[1]).reshape(-1, 257)
            action = self.actor.predict([x_traffic, x_error])
            observation, reward, done, _ = self.env.step(action, self.env.pointer)
        ...
        df.to_csv('model/%d-realY6p%d.csv'%(episode,self.env.predstep), header=False, index=False)
        
        df = pd.DataFrame(np.array(self.env.set_predY).reshape(-1, 257))
        df.to_csv('model/%d-seknn-predY6p%d.csv'%(episode,self.env.predstep), header=False, index=False)
        
        df = pd.DataFrame(np.array(self.env.set_predY_).reshape(-1, 257))
        df.to_csv('model/%d-reinforced-predY6p%d.csv'%(episode,self.env.predstep), header=False, index=False)
...
if __name__ == '__main__':
    ...
    model.save_history(history, '/home/jinan/AutoKNN/model/%d-ddpg.csv'%episode)
    model.play(episode)
